...
log_vnpay_debug_data(
    "IPN_URL - RECEIVED DATA",
    [
        "get_params" => $_GET,
            ...
            if ($order_total_amount_db == $vnp_Amount_from_vnpay) {
                ...
                if ($order["payment_status"] == 'pending' || $order["payment_status"] == '') {
                    ...
                    if ($inputData['vnp_ResponseCode'] == '00') {
                        ...
                        if ($order['buyer_id']) {
                            try {
                                // Sử dụng CartModel để xóa các sản phẩm đã mua khỏi giỏ hàng
                                // require_once(__DIR__ . '/../../Models/cart/CartModel.php'); // Already included at the top
                                $cartModel = new CartModel($pdo);

                                // Lấy danh sách product_id của các sản phẩm đã mua trong đơn hàng này
                                $stmt_get_product_ids = $pdo->prepare("SELECT product_id FROM order_items WHERE order_id = ?");
                                $stmt_get_product_ids->execute([$order['id']]);
                                $bought_product_ids = $stmt_get_product_ids->fetchAll(PDO::FETCH_COLUMN);

                                if (!empty($bought_product_ids)) {
                                    $cartModel->removeBoughtItems($order['buyer_id'], $bought_product_ids);
                                    error_log("IPN: Removed " . count($bought_product_ids) . " items from cart for user " . $order['buyer_id'] . " after successful payment for order: {$order['id']}");
                                    log_vnpay_debug_data("IPN_URL - CART ITEMS REMOVED", ["order_number" => $order_number, "buyer_id" => $order['buyer_id'], "removed_product_ids" => $bought_product_ids]);
                                } else {
                                    log_vnpay_debug_data("IPN_URL - NO CART ITEMS TO REMOVE", ["order_number" => $order_number, "buyer_id" => $order['buyer_id']]);
                                }
                            } catch (Exception $e) {
                                error_log("IPN: Error removing cart items for user {$order['buyer_id']}: " . $e->getMessage());
                                log_vnpay_debug_data("IPN_URL - CART REMOVE ERROR", ["order_number" => $order_number, "buyer_id" => $order['buyer_id'], "error" => $e->getMessage()]);
                            }
                        }
