...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div>
            ...
            <div class="flex gap-4 mt-4">
                ...
                <div class="flex items-center gap-2 px-3 py-1.5 bg-green-50 dark:bg-green-900/20 rounded-lg">
                    ...
                    <span class="text-sm font-medium text-green-700 dark:text-green-300"><?= $activeCoupons ?> đang hoạt
                        động</span>
                </div>
                <div class="flex items-center gap-2 px-3 py-1.5 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <span class="text-sm font-medium text-purple-700 dark:text-purple-300"><?= $totalUsed ?> lượt
                        dùng</span>
                </div>
            </div>
        </div>
    ...
    <?php if (empty($coupons)): ?>
        <!-- Empty State -->
        <div
            class="bg-white dark:bg-gray-800 shadow-sm rounded-2xl border border-gray-200 dark:border-gray-700 p-12 text-center">
            <div
                class="mx-auto w-24 h-24 bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 rounded-full flex items-center justify-center mb-4">
    ...
    <?php else: ?>
        <!-- Table -->
        <div
            class="bg-white dark:bg-gray-800 shadow-sm rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        <?php foreach ($coupons as $coupon): ?>
                            ...
                            ?>
                            <tr
                                class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all duration-200 <?= $coupon['status'] ? '' : 'opacity-60' ?>">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span
                                            class="inline-flex items-center px-3 py-1.5 rounded-lg text-sm font-bold bg-gradient-to-r from-gray-100 to-gray-200 text-gray-800 font-mono border-2 border-dashed border-gray-300 dark:from-gray-700 dark:to-gray-600 dark:text-white dark:border-gray-500">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <?php if ($coupon['discount_type'] === 'percent'): ?>
                                            <div
                                                class="px-3 py-1.5 bg-gradient-to-r from-green-500 to-emerald-600 rounded-lg shadow-md">
                                                <span
                                                    class="text-white font-bold text-sm"><?= number_format($coupon['discount_value']) ?>%</span>
                                            </div>
                                            <?php if (!empty($coupon['max_discount_amount']) && $coupon['max_discount_amount'] > 0): ?>
                                                <div class="mt-1 px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-center">
                                                    <span class="text-xs text-gray-600 dark:text-gray-300">Tối đa:
                                                        <?= number_format($coupon['max_discount_amount']) ?>đ</span>
                                                </div>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <div
                                                class="px-3 py-1.5 bg-gradient-to-r from-blue-500 to-cyan-600 rounded-lg shadow-md">
                                                <span
                                                    class="text-white font-bold text-sm"><?= number_format($coupon['discount_value']) ?>
                                                    ₫</span>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-600 dark:text-gray-300">
                                        ...
                                        <span
                                            class="font-bold text-indigo-600 dark:text-indigo-400"><?= number_format($coupon['min_order_value']) ?>
                                            ₫</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <div class="flex-1">
                                            <div class="flex items-baseline gap-1">
                                                <span
                                                    class="text-lg font-bold text-gray-900 dark:text-white"><?= $coupon['used_count'] ?></span>
                                                <span class="text-gray-400">/</span>
                                                <span
                                                    class="text-sm text-gray-500 dark:text-gray-400"><?= $coupon['usage_limit'] > 0 ? $coupon['usage_limit'] : '∞' ?></span>
                                            </div>
                                            <?php if ($coupon['usage_limit'] > 0): ?>
                                                <?php $percentage = min(100, ($coupon['used_count'] / $coupon['usage_limit']) * 100); ?>
                                                <div
                                                    class="w-20 h-1.5 bg-gray-200 dark:bg-gray-700 rounded-full mt-1 overflow-hidden">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button onclick="toggleStatus(<?= $coupon['id'] ?>)"
                                        ...
                                        <span
                                            class="pointer-events-none inline-block h-6 w-6 rounded-full bg-white shadow-lg transform ring-0 transition-all duration-300 ease-in-out <?= $coupon['status'] ? 'translate-x-7' : 'translate-x-0' ?>">
        ...
        <div
            ...
            <form id="createCouponForm" onsubmit="submitCreate(event)">
                ...
                <div class="bg-white dark:bg-gray-800 px-6 py-6">
                    <div class="space-y-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Mã Code
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Loại
                                    giảm giá</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Giá trị
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Giảm tối đa
                                (₫)</label>
                            <input type="number" name="max_discount_amount" min="0"
                                class="block w-full rounded-xl border-2 border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white px-4 py-3 text-sm transition-all"
                                placeholder="Để trống nếu không giới hạn">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Đơn hàng
                                tối thiểu (₫)</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Ngày
                                    bắt đầu</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Giới hạn
                                lượt dùng</label>
                        ...
                        <div class="flex items-center p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl">
                            ...
                            <label class="ml-3 block text-sm font-medium text-gray-900 dark:text-gray-300">Kích hoạt
                                ngay</label>
                        </div>
                    </div>
                </div>
